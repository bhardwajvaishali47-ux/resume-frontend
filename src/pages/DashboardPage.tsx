...
export default function DashboardPage() {
  ...
  const handleChat = async (override?: string) => {
    ...
    try {
      ...
      // Auto-detect if coach rewrote bullets — show hint
      ...
      if (hasRewrites) {
        setEnhanceDone(false) // reset so user can re-generate
        setPdfBase64('')
        setEnhancedSummary('')
      }
    } catch (e: any) {
  ...
  // ── Enhanced Resume: extract bullets from chat + generate PDF ─────────────
  const handleEnhanceResume = async () => {
    if (messages.length === 0) {
      setEnhanceError('Chat with the AI coach first to rewrite your bullets.')
      return
    }
    ...
    <div style={{ background: '#101319', color: '#e1e2eb', fontFamily: "'Inter',sans-serif", minHeight: '100vh' }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn-teal { background: linear-gradient(135deg,#43e1ba,#00c4a0); color:#002e25; font-weight:700; border:none; cursor:pointer; transition:transform .15s, box-shadow .15s; }
        .btn-teal:hover  { transform:scale(1.02); box-shadow:0 8px 20px -4px rgba(67,225,186,.35); }
        .btn-teal:active { transform:scale(0.98); }
        .btn-teal:disabled { opacity:.55; cursor:not-allowed; transform:none; }
      ...
      <nav style={{ background:'#101319', borderBottom:'1px solid rgba(91,64,62,.15)', display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0 20px', height:56, position:'sticky', top:0, zIndex:50 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <span style={{ fontSize:13, color:'#e1e2eb' }}>{user?.full_name || user?.name || user?.email?.split('@')[0] || 'User'}</span>
          ...
          <button onClick={handleLogout} className="mono" style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 10px', borderRadius:7, background:'none', border:'1px solid rgba(91,64,62,.2)', color:'#E4BDBA', cursor:'pointer', fontSize:10, textTransform:'uppercase' }}>↩ Logout</button>
        </div>
      </nav>
        ...
        <aside style={{ background:'#191C22', width:SIDEBAR_W, position:'fixed', left:0, top:56, height:'calc(100vh - 56px)', display:'flex', flexDirection:'column', padding:'20px 0', boxShadow:'4px 0 20px rgba(0,0,0,.4)', zIndex:40, transition:'width .25s cubic-bezier(0.4,0,0.2,1)', overflow:'hidden' }}>
          ...
          <nav style={{ flex:1, padding:'0 8px', display:'flex', flexDirection:'column', gap:3 }}>
            {([
              ...
              <button key={item.tab} className={`nav-item ${activeTab===item.tab?'active':'inactive'}`}
                onClick={() => setActiveTab(item.tab)} title={!sidebarOpen ? item.label : undefined}
          ...
          <div style={{ padding:'0 8px', marginTop:'auto', display:'flex', flexDirection:'column', gap:3 }}>
            {sidebarOpen && <div className="mono" style={{ fontSize:8, textTransform:'uppercase', color:'#4D5A72', padding:'0 12px', marginBottom:4 }}>Account</div>}
            <button className="nav-item inactive" style={{ justifyContent:sidebarOpen?'flex-start':'center' }} title={!sidebarOpen?'Settings':undefined}><span style={{ fontSize:15 }}>⚙️</span>{sidebarOpen&&<span>Settings</span>}</button>
            <button className="nav-item inactive" style={{ justifyContent:sidebarOpen?'flex-start':'center' }} title={!sidebarOpen?'Support':undefined}><span style={{ fontSize:15 }}>❓</span>{sidebarOpen&&<span>Support</span>}</button>
            <button className="btn-coral" onClick={handleReset} style={{ marginTop:12, padding:sidebarOpen?'10px':'8px', borderRadius:8, fontSize:sidebarOpen?12:16, width:'100%' }} title={!sidebarOpen?'New Analysis':undefined}>
          ...
          {!analyzed && (
            <div className="fade-up" style={{ maxWidth:640, margin:'0 auto' }}>
              <div style={{ marginBottom:28 }}>
                <h1 style={{ fontSize:22, fontWeight:800, color:'#e1e2eb', marginBottom:6, letterSpacing:'-0.02em' }}>Start Your Analysis</h1>
                <p style={{ fontSize:14, color:'#ab8886' }}>Upload your resume and paste the job description to get your personalised match score.</p>
              </div>
              {analyzeError && <div style={{ marginBottom:14, padding:14, borderRadius:10, background:'rgba(255,180,171,.1)', border:'1px solid rgba(255,180,171,.2)', color:'#ffb4ab', fontSize:13 }}>⚠ {analyzeError}</div>}
              ...
              <button onClick={handleAnalyze} disabled={analyzing} className="btn-coral" style={{ width:'100%', padding:'14px', borderRadius:10, fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                {analyzing?<><svg width="16" height="16" viewBox="0 0 24 24" style={{ animation:'spin .8s linear infinite' }}><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2.5" fill="none" strokeDasharray="40" strokeDashoffset="10" strokeLinecap="round"/></svg>Analysing…</>:'🔍 Analyse My Resume'}
              </button>
            </div>
          )}
                  ...
                  <div className="card" style={{ display:'flex', flexDirection:'column', height:480 }}>
                    ...
                    <div style={{ padding:'10px 14px', background:'#101319', borderTop:'1px solid rgba(91,64,62,.1)', borderRadius:'0 0 12px 12px', flexShrink:0 }}>
                      <div style={{ position:'relative' }}>
                        <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();handleChat()}}} placeholder="Ask your career coach…" disabled={chatLoading}
                        ...
                        <button onClick={()=>handleChat()} disabled={chatLoading||!chatInput.trim()} style={{ position:'absolute', right:6, top:'50%', transform:'translateY(-50%)', padding:7, borderRadius:7, background:'none', border:'none', color:'#FFB3AE', cursor:'pointer', fontSize:16, opacity:chatLoading||!chatInput.trim()?0.4:1 }}>↑</button>
                      </div>
                    </div>
                  </div>
                    ...
                    <div style={{ padding:22 }}>
                      ...
                      {activeTab==='resume'&&(
                        <div className="fade-up" style={{ display:'flex', flexDirection:'column', gap:16 }}>
                          ...
                          {!enhanceDone&&(
                            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                              {[
                                ...
                                { n:2, color:'#FFB347', bg:'rgba(255,179,71,.1)', text:'The coach rewrites them in the chat above using your resume and job context' },
                                { n:3, color:'#43e1ba', bg:'rgba(67,225,186,.1)', text:'Click "Apply & Download PDF" below — the app extracts the rewrites and generates your enhanced resume' },
                              ].map(step=>(
                            ...
                            <div>
                              ...
                              <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                                {[
                                  'Rewrite my BT Group bullets for this role',
                                  'Rewrite my professional summary',
                                  'Strengthen my most recent role bullets',
                                  'Make my experience sound more product-led',
                                ].map(p=>(
                                  <button key={p} onClick={()=>{ handleChat(p); }}
                                    style={{ padding:'7px 12px', borderRadius:999, background:'#272a31', border:'1px solid rgba(91,64,62,.2)', color:'#E4BDBA', fontSize:11, cursor:'pointer', fontFamily:"'JetBrains Mono',monospace", whiteSpace:'nowrap', transition:'all .15s' }}>
                                    {p}
                                  </button>
                                ))}
                              </div>
                          ...
                          {/* Error */}
                          {enhanceError&&(
                            <div style={{ padding:12, borderRadius:8, background:'rgba(255,180,171,.1)', border:'1px solid rgba(255,180,171,.2)', color:'#ffb4ab', fontSize:12 }}>
                              ⚠ {enhanceError}
                            </div>
                          )}
                          ...
                          {!enhanceDone&&(
                            <button onClick={handleEnhanceResume} disabled={enhancing||messages.length===0} className="btn-coral"
                              ...
                              {enhancing
                                ...
                                : messages.length===0
                                  ? '💬 Chat with the coach first to rewrite your bullets'
                                  : '🚀 Apply Rewrites & Generate PDF'
                              }
                            </button>
                          )}
